...
func init() {
	...
	flag.BoolVar(&onlyHeader, "I", false, "don't read body or request")
...
func visit(url *url.URL) {
	...
	trace := &httptrace.ClientTrace{
		...
		ConnectDone: func(net, addr string, err error) {
			...
			printf("\n%s%s\n", color.GreenString("Connection to "), color.CyanString(addr))
		},
	...
	case "https":
		...
		// Beacuse we create a custom TLSClientConfig, we have to opt-in to HTTP/2.
	...
	client := &http.Client{
		...
		CheckRedirect:	func(req *http.Request, via []*http.Request) error {
			// always refuse to follow redirects, visit dose that
...
func getFilenameFromHeader(headers http.Header) string {
	// if the Conetne-Disposition header is set parse it
...
// readResponseBody returns an informational message about the disposition of the response body's contents.
func readResponseBody(req *http.Request, resp *http.Response) string {
	...
	if saveOutput || outputFile != "" {
		...
		if saveOutput {
			...
			// otherwise fail back to the RequestURI
